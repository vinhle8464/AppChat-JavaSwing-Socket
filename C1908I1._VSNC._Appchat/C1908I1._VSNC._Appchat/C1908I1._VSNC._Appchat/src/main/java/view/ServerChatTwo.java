...
public class ServerChatTwo {
	...
	public ServerChatTwo() {}
	public ServerChatTwo(int port, String account, InetAddress hostuser) {
	...
	public void execute() throws IOException {
		...
		if(serversocket == null) {
			...
			for (String port : accoundao.selAllport()) {
					...
					public void run() {
						try {
							...
							while(true) {
								...
								ServerChatTwo.listsocket.add(socket);
	...
	public static void main(String[] args) throws IOException {
		ServerChatTwo.listsocket = new ArrayList<>();
		
		var server = new ServerChatTwo(15797, username, host);
		server.execute();
	}
		...
		try {
			...
			while(true) {
				...
				if(sms.contains("exit")) {
					...
					ServerChatTwo.listsocket.remove(socket);
				...
				for (Socket item : ServerChatTwo.listsocket) {
